...
- Complex-script emphasis (header) rendering: Unity's synthetic `FontStyle.Bold` on the dynamic OS overlay font has no real bold face under Proton/IL2CPP, so it algorithmically thickens glyphs — filling the tight loops/counters of CJK/kana/Hangul/**Thai** and rendering bold section headers unreadable (JA/TH i18n P0 bug). Fix: emphasis headers carry their weight via a **real bold face** (not synthetic bold, not a size bump), script-aware (`GlyphScript` + `TextBinding` + `UGuiPrimitives.ApplyEmphasisFont`): **Latin** → real `FontStyle.Bold`; **Thai** → a real bold Thai FACE at `FontStyle.Normal` — `WindowThemeAssets.ThaiBoldFont` = **`Stellar Thai Bold`**, the loopless Noto Sans Thai Bold renamed to a unique normal-weight family and installed as a **HOST font** (`~/.local/share/fonts`; `Font.CreateDynamicFontFromOSFont` under Proton reads the host's `fc-list` fonts, NOT the Wine prefix — a host *bold family name* also does not resolve, only a distinct normal-weight family does); falls back to `Noto Looped Thai Bold` then the regular Thai face. **CJK/kana/Hangul** have no reachable bold face and read as regular weight. Re-derived per string change, so it tracks live language switches. (The `Stellar Thai Bold` font ships + auto-installs for all users in a follow-up.)
