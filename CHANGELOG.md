...
### Developer notes
- New `Stellar.Abstractions.Domain.ActorState` enum (`Dead`=9, `Breaking`=23, `Unknown`=0 for any other/future wire value) and `CombatEvent.EntityStateChanged(TimestampMs, TargetId, ActorState)`, riding the existing `ICombatEvents` stream — no service interface gains a member, so STELLAR0005's 8-member ceiling is untouched. Field result (2026-07-28, round 2): the originally-spec'd leaf patches (`Panda.ZGame.EntityCtrlDead.OnEnter`, `Panda.ZGame.ZStateBreaking.OnEnter`) install cleanly but never fired across three owner dungeon clears with mob kills — see `recon/entity-state-death-signal-notes.md`. Primary signal is now HarmonyX postfixes on `Panda.ZGame.ZStateMachine.onStateChanged`/`EnterState` (fire on every actor transition; the state argument is unboxed and filtered against `ActorState` — via `ActorStateMapper`, no reflection — BEFORE any reflected `Host`/`Uuid` getter runs, since these fire orders of magnitude more often than a death leaf). `ZStateDead.OnEnter` and the two original leaf sites stay installed one more round as a diagnostic (each site tags its own ungated log lines so one run shows which are actually live); duplicate raises across sites for the same transition are de-duplicated by (entity, state) within a short window. Every site resolves/installs independently and degrades to "signal off" (logged) rather than throwing if a type/accessor is missing after a future game patch — see `PandaEntityStateProbe`. 2026-07-28 entity-state-death-signal spec; retires the HP-inference gone-timeout for `ArchiveReason.BossKill` on the plugin side (follow-up work, not in this release).
