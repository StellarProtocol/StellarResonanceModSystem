...
- Complex-script emphasis (bold) rendering: Unity's synthetic `FontStyle.Bold` on the dynamic OS overlay font has no real bold face under Proton/IL2CPP, so it algorithmically thickens glyphs — which fills the tight loops/counters of CJK/kana/Hangul/**Thai** and renders bold section headers unreadable (JA/TH i18n P0 bug). Emphasised window text + window/overlay titles are now script-aware (`GlyphScript` + `TextBinding` + `UGuiPrimitives.ApplyEmphasisFont`): **Latin** keeps real `FontStyle.Bold`; **Thai** uses a real bold Thai FACE (`WindowThemeAssets.ThaiBoldFont` = `Noto Looped Thai Bold`, applied with `FontStyle.Normal` — crisp readable bold, falls back to the regular Thai face if the family is absent); **CJK/kana/Hangul** (no name-reachable bold face) use a larger crisp regular. Re-derived per string change, so it tracks live language switches.
