...
## [Unreleased]
### Added
- Plugin bars can now be made taller and wider (or stretched to fill their row), given a larger label, and can show that label centred right on top of the bar — so plugins can build a big, prominent bar like a target's HP bar. Works the same whether the bar is a free-floating on-screen bar or sits inside a plugin window.
- Plugin bars can also use a flatter "meter" look — a value label on each end of the bar and an optional soft moving shine — so plugins can match the Combat Meter's damage-bar style. The normal bar look is unchanged and stays the default.
### Developer notes
...
- `BarElement` additionally gains three `init`-only members for an opt-in meter render — `BarStyle Style` (new enum, `Default`=0 / `Modern`), `Func<string>? SecondaryLabel`, and `bool Sheen` — leaving the positional constructor untouched, so this too is **source- AND binary-compatible**; unset `Style` defaults to `Default` and hits the pre-existing render path byte-for-byte. `BuildBar` branches to `HudElementBuilder.Meter.cs` when `Style == Modern`: a flat translucent track (`MeterTrackBg`) + a flat role-coloured anchor-clipped fill (`anchorMax.x` + `RectMask2D`, driven by a `MeterBarBinding` in `token.MeterBars`, mirroring the CombatMeter row) + dual full-width overlay labels (`Label` left / `SecondaryLabel` right). `Sheen = true` adds a procedural 64×4 texture swept per-frame through a new HUD pulse hook (`token.Pulses` → `HudBarAnimator.Step` accumulates `dt`; `HudRenderer.Destroy` removes a token's pulses by reference, and `DropCanvas` disposes the shared `_sheenTex` for `WindowRenderer` parity). Honours all geometry fields except `LabelInside` (meaningless with dual labels). Ported from `WindowBuilder.MeterRow`/`Preview`; no runtime dependency on `WindowBuilder`.
- **Both renderers now honour these members** — the plugin **window** bar (`WindowBuilder.BuildBar`, for `BarElement`s inside a `Windows.Register` window) previously ignored every new field and always drew the plain default bar. It now mirrors the HUD path: the Default branch honours the geometry fields, and a `Style == Modern` branch (`BuildBarModernWindow` in `WindowBuilder.Preview.cs`) renders the meter look by **reusing WindowBuilder's own CombatMeter-row primitives** (`MeterTrackBg`, `SheenTexture()`/`DriveSheen()`, `AddOverlayText()`, `token.Pulses`/`_registerPulse`, and `BarBinding`/`TextBinding`) — no new pulse/animator/texture plumbing and nothing shared with the HUD builder. A window `new BarElement(f, color, label) { Style = BarStyle.Modern, Sheen = true, Height = 28, FillWidth = true, SecondaryLabel = pct }` now renders identically to the HUD.
- **Rename:** the enum member `BarStyle.Meter` is now `BarStyle.Modern`. **Source-breaking** for any plugin that referenced it by name (recompile against the new SDK), but **binary-compatible** — the value is unchanged (`Default`=0, `Modern`=1), so an already-compiled plugin carrying the integer keeps working.
- **The HUD rendering path was unified into the window renderer — there is now ONE UI path.** The separate HUD toolkit (`IHudHost`/`HudSpec`/`HudAnchor`, `HudService`, `HudRenderer`, `HudElementBuilder`(+`.Meter`), `HudBarAnimator`) is **removed**; the window renderer (`WindowBuilder`/`WindowRenderer`) is the single builder for both windows and on-screen overlays. It gains an opt-in `WindowSpec.Surface = SurfaceStyle.HudOverlay` (new `enum SurfaceStyle { Menu, HudOverlay }`, default `Menu` → every existing window byte-identical) that reproduces the old HUD look — always-shadowed twin text honouring `FontSize`/`DynamicFontSize`/`ShadowDistance`, the rounded 9-slice bar with per-frame smoothed fill, and the transparent pill chip — by reusing `HudThemeAssets` (which now lives on the window renderer). The `BarStyle.Modern` note above therefore reads historically: that render now exists only in `WindowBuilder`. **Breaking for the (few) plugins that registered a HUD via `IPluginServices.Hud.Register(HudSpec)`** — they move to `IPluginServices.Windows.Register(new WindowRegistration(new WindowSpec(..., WindowCategory.HUD, WindowPanelStyle.Borderless){ Surface = SurfaceStyle.HudOverlay, Draggable = true, EditModeDragOnly = true, ShouldRender = ... }, root))`; the element `Root` tree is unchanged (the `HudElement` records are shared and kept). The three in-repo/first-party HUD consumers (PlayerHUD, RaidManager countdown, DebugInfo) were migrated in lockstep and render pixel-identically. `IThemeHudColors` is retained (Toast + the `HudOverlay` bake still use it).
