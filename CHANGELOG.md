...
### Developer notes
- New partial `WindowBuilder.MeterRowLabelFade`: two `RawImage` fades (64 px × font scale) under the Primary/Secondary bar labels, children of the BAR (not the width-clipped fill), drawn above fill + sheen and below the texts; one shared 64×4 alpha-ramp texture (full over the first 50 %, then SmoothStep to 0), mirrored via `uvRect` for the right end; the right fade toggles with `SecondaryGo` (`MeterRowBinding.SecondaryFadeGo`). Strength is per fill: `LabelFadeTint` lerps the alpha 0.12 → 0.52 with the fill's relative luminance and the binding retints both fades whenever the fill colour changes — faint on role red/blue and the dark crest colours, ≈ 0.40 on Marksman yellow (second in-game round, owner: "text look smooth but shadow is too much" against a fixed 0.58). Measured trigger: with CombatMeter 2.10.0's class palette the fill can be `#f2dc3a` / `#5ccf36` / `#f5761c`, where white text has a WCAG contrast of 1.4–2.0. A uGUI `Outline` on the labels was deployed first and rejected in-game by the owner ("readable but the text looks rough, no smooth at all") — legacy-Text outlines are four hard-edged 1 px glyph copies. `FrameworkVersion.Value` 2.7.5; no API change. Design record: devkit `docs/superpowers/specs/2026-09-09-game-class-palette-and-gauge-color-design.md`.
