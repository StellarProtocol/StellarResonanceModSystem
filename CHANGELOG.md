...
- Complex-script emphasis (bold) gate: the dynamic OS overlay font has no real bold face under Proton/IL2CPP, so `FontStyle.Bold` is synthesised — and that faux-bold mangles CJK/kana/Hangul/Thai glyphs (JA/TH bold section headers rendered unreadable). `GlyphScript.HasSyntheticBoldRisk(string)` (pure, unit-tested) detects those scripts; emphasised uGUI text (`UGuiPrimitives.EmphasisStyle`, applied by `TextBinding` on every text change, the window/overlay titles, the HUD bar prefix, and the chart legend) now keeps real bold for Latin but drops to regular weight for complex scripts, which the OS-font fallback chain renders correctly. Re-derived per string change, so it tracks live language switches and dynamic HUD content.
