...
- Complex-script emphasis (header) rendering: emphasis headers use Unity's synthetic `FontStyle.Bold` for every script (`TextBinding` + `UGuiPrimitives.ApplyEmphasisFont`), consistent with English. This is a documented engine limit — under Proton/IL2CPP `Font.CreateDynamicFontFromOSFont` will NOT load a real bold FACE (verified across a host bold family name, a distinct normal-weight family installed both in the Wine prefix and as a host `fc-list` font — none render); the only bold the overlay renders is Unity's algorithmic thickening. Latin tolerates it cleanly; complex scripts (CJK/Thai) thicken less crisply (their tight loops/counters fill in), which is inherent to synthetic bold and cannot be made crisp without a real bold face the engine can't load. `GlyphScript` (script detection) remains for the secondary emphasis paths.
