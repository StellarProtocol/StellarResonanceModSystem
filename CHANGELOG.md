...
- Complex-script emphasis (header) rendering: Unity's synthetic `FontStyle.Bold` on the dynamic OS overlay font has no real bold face under Proton/IL2CPP, so it algorithmically thickens glyphs — filling the tight loops/counters of CJK/kana/Hangul/**Thai** and rendering bold section headers unreadable (JA/TH i18n P0 bug). Fix: emphasis headers are now **larger** (18 vs 14 body) — the size is the reliable standout cue in *every* language even when no bold face resolves — plus real bold weight where available. Script-aware (`GlyphScript` + `TextBinding` + `UGuiPrimitives.ApplyEmphasisFont`): **Latin** → larger + real `FontStyle.Bold`; **Thai** → larger + a real bold Thai FACE at `FontStyle.Normal` (`WindowThemeAssets.ThaiBoldFont` = **`Stellar Thai Bold`**, the loopless Noto Sans Thai Bold renamed to a unique normal-weight family and installed into the game's Wine-prefix Fonts dir so `CreateDynamicFontFromOSFont` resolves it — a host bold family name did NOT resolve under Proton; falls back to `Noto Looped Thai Bold` then the regular Thai face); **CJK/kana/Hangul** (no bold face) → larger crisp regular. Re-derived per string change, so it tracks live language switches.
