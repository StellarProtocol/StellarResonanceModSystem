...
- Complex-script emphasis (bold) gate: the dynamic OS overlay font has no real bold face under Proton/IL2CPP, so `FontStyle.Bold` is synthesised — and that faux-bold mangles CJK/kana/Hangul/Thai glyphs (JA/TH bold section headers rendered unreadable). `GlyphScript.HasSyntheticBoldRisk(string)` (pure, unit-tested) detects those scripts; emphasised uGUI text (`UGuiPrimitives.ApplyEmphasis`, applied by `TextBinding` on every text change + the window/overlay titles) keeps real `FontStyle.Bold` for Latin, but for complex scripts uses `FontStyle.Normal` + a same-colour `Outline` fatten (`EmphasisFattenPx`) so the header still reads **bold and readable** without the synthetic-bold stroke distortion. Re-derived per string change, so it tracks live language switches and dynamic content.
