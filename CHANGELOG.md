...
## [2.7.5] - 2026-09-09
_**2.7.5** (patch) — meter bar numbers get the same dark outline the stat HUD uses, so they stay readable on bright bar colours. Infrastructure-only; no API change, binary-compatible with all existing plugins._
### Fixed
- The numbers drawn on a damage-meter bar now have the same thin dark outline as the stat HUD's text, so they stay readable when the bar is a bright colour (for example the yellow, green and orange class colours in CombatMeter's new "Bar colour: Class" option).
### Developer notes
- `WindowBuilder.MeterRow.AddOverlayText` applies the framework's existing readability outline to the Primary/Secondary bar labels — the recipe `TextElement(Shadow: true)` already uses for chrome-less overlays (StatInspector's stat HUD): uGUI `Outline`, black @ 0.85, `effectDistance (1.1, -1.1)`; now factored into `UGuiPrimitives.AddReadabilityOutline` and shared by both call sites. Measured trigger: with CombatMeter 2.10.0's class palette the fill can be `#f2dc3a` / `#5ccf36` / `#f5761c`, where white text has a WCAG contrast of 1.4–2.0. Iterations rejected in-game on the way (all in this PR's history): an `Outline` at exactly `(1, -1)` ("readable but the text looks rough" — pixel-aligned copies of a 10 px bold glyph stair-step; the stat HUD's 1.1 px sub-pixel offset is what softens it), then a fixed and then a luminance-adaptive dark fade under the values ("shadow is too much" / "I want the outline, smooth"). `FrameworkVersion.Value` 2.7.5; no API change. Design record: devkit `docs/superpowers/specs/2026-09-09-game-class-palette-and-gauge-color-design.md`.
