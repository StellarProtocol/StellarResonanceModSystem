...
/// stronger accent fill. A click anywhere on the row fires <paramref name="OnClick"/>. <paramref name="OnHover"/>
/// (optional) fires with <c>true</c> when the pointer enters the row and <c>false</c> when it leaves (ticker-driven;
/// never fires in the static sandbox) — use it for hover-preview panes. Composes inside a List/Column like any leaf.</summary>
public sealed record SelectableElement(HudElement Child, Action OnClick, Func<bool>? Selected = null,
    Action<bool>? OnHover = null) : HudElement;
