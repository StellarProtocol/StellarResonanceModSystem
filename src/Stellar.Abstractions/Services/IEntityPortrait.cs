...
/// <summary>Camera-interaction controls for the entity portrait (drag/scroll/resize).</summary>
public interface IEntityPortraitView
{
...
public interface IEntityPortrait : IEntityPortraitView
{
    ...
    /// <summary>When false (default), the portrait hides the subject's weapon model; true shows it.
    /// Global to the portrait service (applies to whatever subject is shown).</summary>
    bool ShowWeapon { get; set; }

    /// <summary>Portrait light preset: -1 = Scene (environment-lit, no override); 0=EarlyMorning, 1=Morning,
    /// 2=HighNoon (default), 3=Sunset, 4=Night. Values map to the game's baked creature-RT light presets.</summary>
    int LightPreset { get; set; }
