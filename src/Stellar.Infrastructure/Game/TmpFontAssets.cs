...
/// <summary>
/// Process-shared TextMeshPro font assets for styled overlay text (i18n typography: real bold in every
/// script, plus italic/underline/strikethrough). <see cref="UiRegular"/>/<see cref="UiBold"/> are merged
/// Noto Sans + Noto Sans Thai faces SHIPPED as embedded resources, extracted beside the framework DLLs
/// and loaded BY FILE PATH. Measured fact (font probe, 2026-08-18, docs/recon/overlay-font-glyph-coverage.md):
/// Unity under Proton enumerates only the Wine prefix's fonts — never the host's — so an OS-name lookup
...
{
    private static TMP_FontAsset? _uiRegular;
    private static TMP_FontAsset? _uiBold;
    private static TMP_FontAsset? _cjkRegular;
    private static TMP_FontAsset? _cjkBold;
    private static bool _tried;

    /// <summary>Regular Latin+Thai face (shipped, machine-independent). Null when creation failed.</summary>
    public static TMP_FontAsset? UiRegular { get { Ensure(); return _uiRegular; } }
    ...
    /// <summary>Regular CJK/kana/Hangul face from a system family. Null when no candidate resolved.</summary>
    public static TMP_FontAsset? CjkRegular { get { Ensure(); return _cjkRegular; } }
    ...
    private static void Ensure()
    {
        if (_tried) return;
        _tried = true;
        try { _uiRegular = CreateShipped("Stellar.Infrastructure.Resources.StellarUIThai-Regular.ttf", "StellarUIThai-Regular.ttf"); }
        catch { _uiRegular = null; }
        try { _uiBold = CreateShipped("Stellar.Infrastructure.Resources.StellarUIThaiBold.ttf", "StellarUIThaiBold.ttf"); }
        catch { _uiBold = null; }
        try { _cjkRegular = CreateCjk("Regular"); } catch { _cjkRegular = null; }
        try { _cjkBold = CreateCjk("Bold"); } catch { _cjkBold = null; }
    }

    private static TMP_FontAsset? CreateShipped(string resourceName, string fileName)
    {
        var path = ExtractShippedFont(resourceName, fileName);
    ...
    private static string? ExtractShippedFont(string resourceName, string fileName)
    {
